...
export const sendVerificationOtp = async (
	...
	try {
		await resend.emails.send({
			...
			to: email,
			subject: "Verify your email - SmartHireHub",
			html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2>Welcome to SmartHireHub, ${name}!</h2>
...
export const sendWelcomeEmail = async (
	...
	try {
		await resend.emails.send({
			from: config.FROM_EMAIL,
			to: email,
			subject: "Welcome to SmartHireHub!",
			html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2>Welcome aboard, ${name}! 🎉</h2>
                    <p>Your email has been verified successfully.</p>
                    <p>You now have full access to SmartHireHub. Start exploring opportunities and begin your career journey!</p>
...
export const sendPasswordResetEmail = async (
	...
	try {
		await resend.emails.send({
			from: config.FROM_EMAIL,
			to: email,
			subject: "Reset your password - SmartHireHub",
			html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
export const sendPasswordChangedEmail = async (
	...
	try {
		await resend.emails.send({
			from: config.FROM_EMAIL,
			to: email,
			subject: "Your password was changed - SmartHireHub",
			html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
