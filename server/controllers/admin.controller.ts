...
export const getDashboardStats = async (_req: Request, res: Response) => {
    try {
        ...
        const [
            ...
            prisma.user.count({ where: { role: "USER", status: "ACTIVE", isDeleted: false } }),

            // Companies
            prisma.user.count({ where: { role: "COMPANY", isDeleted: false } }),
            prisma.user.count({ where: { role: "COMPANY", status: "ACTIVE", isDeleted: false } }),

            // HR
            prisma.user.count({ where: { role: "HR", isDeleted: false } }),
            prisma.user.count({ where: { role: "HR", status: "ACTIVE", isDeleted: false } }),
        ...
        const userRegistrationsByDay = last7Days.map(day => {
            const count = userRegistrations.filter(r =>
        ...
        const applicationsByDay = last7Days.map(day => {
            const count = applicationsOverTime.filter(a =>
                a.appliedAt.toISOString().split('T')[0] === day
...
export const getAllJobs = async (req: Request, res: Response) => {
    try {
        ...
        logger.info(`Successfully fetched ${jobs.length} jobs out of ${total} total`);
