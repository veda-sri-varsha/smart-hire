...
import { getAllJobs, deleteJobById } from "@/api/admin";
    ...
    const fetchJobs = async (page = 1) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const errorMsg = response.error || response.message || "Failed to load jobs";
                setError(errorMsg);
            }
        } catch (err: any) {
        ...
        <div className={styles.container}>
            <aside className={styles.sidebar}>
                ...
                <nav className={styles.nav}>
                    ...
                    <Link to="/admin/jobs" className={styles.active}>Jobs</Link>
                    ...
                    <div className={styles.recentSection}>
                        ...
                        <div className={styles.tableWrapper}>
                            <table className={styles.table}>
                                ...
                                <tbody>
                                    {jobs.length === 0 ? (
                                        <tr><td colSpan={8} style={{ textAlign: 'center', padding: '20px' }}>No jobs found.</td></tr>
                                    ) : (
                                        jobs.map((job) => (
                                            <tr key={job.id}>
                                                <td>
                                                    <Link to="/jobs/$jobId" params={{ jobId: job.id }} style={{ color: '#3b82f6', textDecoration: 'none' }}>
                                                        {job.title}
                                                    </Link>
                                                </td>
                                                <td>{job.company.companyName || job.company.name}</td>
                                                <td>{job.location}</td>
                                                <td><span className={styles.badge}>{job.jobType}</span></td>
                                                <td>
